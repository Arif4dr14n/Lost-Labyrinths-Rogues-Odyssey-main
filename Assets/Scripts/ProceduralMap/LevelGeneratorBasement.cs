...
public class LevelGeneratorBasement : MonoBehaviour
{
    ...
    public TileBase spikeleftTile;
    public TileBase spikerightTile;
    ...
    public TileBase spikeleftRuleTile;
    public Tilemap spikeleftTilemap;

    public TileBase spikerightRuleTile;
    public Tilemap spikerightTilemap;
    ...
    private List<Vector3Int> spikeleftTilePos = new();
    private List<Vector3Int> spikerightTilePos = new();
                ...
                for (int y = 0; y < trimmedRoomSize.y; y++)                                                                         
                {
                    ...
                    {   
                        ...
                        if (tilemap.name == "Indicator Map")
                        {
                            ...
                            else if (tile ==spikeleftTile)
                            {
                                spikeleftTilePos.Add(tileSpawnPos);
                            }
                            else if (tile == spikerightTile)
                            {
                                spikerightTilePos.Add(tileSpawnPos);
                            }
    ...
    private void SpawnSpikesLeft()
    {
        // Group Enemy Tiles Based On Adjacent Position
        List<List<Vector3Int>> groupedSpikeLeftTiles = GroupAdjacentTiles(spikeleftTilePos);
        // Debug.Log(groupedSpikeTiles.Count);

        foreach (List<Vector3Int> group in groupedSpikeLeftTiles)
        {
            // Debug.Log(group.Count);
            int tileCount = group.Count;
            ...
            // Assuming gameManager.difficulty is an int between 0 - 100
            float spawnProbability = Mathf.Clamp01(tileCount * spikeSpawnCoefficient * (difficulty / 100f) / 100f);
            bool spawnSpikeLeft = UnityEngine.Random.value <= spawnProbability;

            if (spawnSpikeLeft)
            {
                foreach (var spawnPos in group)
                {
                    spikeleftTilemap.SetTile(spawnPos, spikeleftRuleTile);
                }
            }
        }
    }

    private void SpawnSpikesRight()
    {
        // Group Enemy Tiles Based On Adjacent Position
        List<List<Vector3Int>> groupedSpikeRightTiles = GroupAdjacentTiles(spikerightTilePos);
        // Debug.Log(groupedSpikeTiles.Count);

        foreach (List<Vector3Int> group in groupedSpikeRightTiles)
        {
            // Debug.Log(group.Count);
            int tileCount = group.Count;
            ...
            // Assuming gameManager.difficulty is an int between 0 - 100
            float spawnProbability = Mathf.Clamp01(tileCount * spikeSpawnCoefficient * (difficulty / 100f) / 100f);
            bool spawnSpikeRight = UnityEngine.Random.value <= spawnProbability;

            if (spawnSpikeRight)
            {
                foreach (var spawnPos in group)
                {
                    spikerightTilemap.SetTile(spawnPos, spikerightRuleTile);
                }
            }
        }
    }
