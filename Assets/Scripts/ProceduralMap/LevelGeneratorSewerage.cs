...
public class LevelGeneratorSewerage : MonoBehaviour
{
    ...
    public TileBase disappearingplatformTile;
    ...
    public TileBase disappearingplatformRuleTile;
    public Tilemap disappearingplatformTilemap;
    ...
    private List<Vector3Int> disappearingplatformTilePos = new();
                ...
                for (int y = 0; y < trimmedRoomSize.y; y++)                                                                         
                {
                    ...
                    {   
                        ...
                        if (tilemap.name == "Indicator Map")
                        {
                            ...
                            else if (tile == disappearingplatformTile)
                            {
                                disappearingplatformTilePos.Add(tileSpawnPos);
                            }
    ...
    private void SpawnDisappearingPlatform()
    {
        // Group Enemy Tiles Based On Adjacent Position
        List<List<Vector3Int>> groupedDisappearingPlatformTiles = GroupAdjacentTiles(disappearingplatformTilePos);
        // Debug.Log(groupedSpikeTiles.Count);

        foreach (List<Vector3Int> group in groupedDisappearingPlatformTiles)
        {
            foreach (var spawnPos in group)
            {
                disappearingplatformTilemap.SetTile(spawnPos, disappearingplatformRuleTile);
            }
        }
    }
